...
def __is_valid_response(response, field):
	...
	if 'query' in response and 'results' in response['query']:
		if response['query']['results'] is None:
			return False
		return field in response['query']['results']
...
def get_historical_info(symbol):
	...
	'''# @author Ben Arvan
def get_hist_info(symbol, start, end):
	yql = 'select * from yahoo.finance.historicaldata where symbol = \"%s\" and startDate = \"%s\" and endDate = \"%s\"'\
	%(start, end)
'''
